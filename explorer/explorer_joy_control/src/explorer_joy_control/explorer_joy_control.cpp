...
ExplorerTeleop::ExplorerTeleop():
    ...
    // 车辆前进后退指令
    linear_turn_left_right(joy_msg->left_axes_left_right),
    // 用于重新开启底盘
    ...
    ph_.param("l_scale_init",   l_scale,        0.8);//0.3 0.6 1.6
    ph_.param("l_scale_full",   l_scale_full,   1.5);
    ph_.param("l_scale_mid",    l_scale_mid,    0.8);
    ph_.param("l_scale_slow",   l_scale_slow,   0.5);

    ph_.param("a_scale_init",   a_scale,        0.8);// 0.1 0.4 0.6
    ph_.param("a_scale_full",   a_scale_full,   1.2);
    ph_.param("a_scale_mid",    a_scale_mid,    0.8);
    ...
    if(joy_msg->askForButton(basic_mode_button)){
        if(joy_msg->askForButton(vice_wheel_mode_button)){
            ...
            if (std::fabs(joy_msg->askForAxes(linear_turn_left_right)) > 10e-6) {
                vel_publisher.angular.z= joy_msg->askForAxes(linear_turn_left_right) * l_scale * 0.7;
            }
            ...
            if (std::fabs(joy_msg->askForAxes(angular_left_right)) > 10e-6) {
                vel_publisher.linear.y = joy_msg->askForAxes(angular_left_right) * a_scale;
            }
        }
    }
